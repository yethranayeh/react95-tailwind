...
> Status: Complete | Deps: 04 | Source: `src/Button/Button.tsx` (220 lines)
...
1. **Adopting Shadcn approach**: Using `class-variance-authority` and the `cn` utility for variant management and class merging.
2. **Hatched background**: Uses the `bg-hatched` utility. Active state swaps border to `border-sunken` + applies hatched pattern.
3. **Raised variant text shift**: On press, the "raised" variant shifts text down 1px by reducing padding-top and increasing padding-bottom. This mimics the original 3D button press effect.
4. **Primary border**: `primary` adds a 1px solid border on top of the existing bevel.
5. **`StyledButton` export**: Maintained as an alias for the `Button` component for backward compatibility.
6. **No SC dependency**: `$disabled` transient prop is replaced with plain `disabled`.
...
- [x] Default button renders with raised 3D border, material background, centered text
- [x] Active state shows sunken border + hatched background
- [x] Disabled state shows greyed text + text shadow
- [x] Primary mode shows thick outer border
- [x] All 4 variants render correctly (default, raised, flat, thin)
- [x] All 3 sizes render correctly (28px, 36px, 44px height)
- [x] Square mode renders equal width/height
- [x] FullWidth mode fills container width
- [x] Focus-visible shows dotted outline
- [x] `as="a"` renders an anchor element
- [x] Visual output matches original `react95@4` Button exactly in all state combinations
- [x] `StyledButton` export exists for backward compatibility
